...
// ... [Mantenha a sua função sair(), obterToken(), showToast(), etc. intactas no topo]

// CORREÇÃO DO ERRO NO BUSCARCEP (Guard Clauses)
async function buscarCEP() {
  ...
  try {
    ...
    if (!data.erro) {
      ...
      desmarcarRadios();
      if(document.getElementById("input-numero")) document.getElementById("input-numero").focus();
    }
  } catch(e) { showToast("Erro no ViaCEP", "error"); } 
  finally { if (btn) btn.innerText = "Buscar"; }
}
...
async function deletarEnderecoSalvo(endereco, modo, pedidoId, valorTotal) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      abrirModalEndereco(modo, pedidoId, valorTotal); // Re-renderiza o modal
    }
  } catch(e) { showToast("Erro ao deletar.", "error"); }
}

// ATUALIZAÇÃO NO ABRIR MODAL (Para incluir o botão de deletar)
function abrirModalEndereco(modo, pedidoId = null, valorTotal = 0) {
  // ... [Lógica do modal permanece igual, mas adicione o botão abaixo na lista de endereços]:
  /* data.enderecosSalvos.forEach((end) => {
          const safeEnd = end.replace(/'/g, "\\'");
          lista.innerHTML += `
            <div style="display: flex; gap: 8px;">
               <label> <input type="radio" name="endereco" value="${end}"> ${end} </label>
               <button onclick="deletarEnderecoSalvo('${safeEnd}', '${modo}', '${pedidoId}', ${valorTotal})">🗑️</button>
            </div>`;
     });
  */
}

// ... [Mantenha todo o resto do seu código original intacto abaixo]
